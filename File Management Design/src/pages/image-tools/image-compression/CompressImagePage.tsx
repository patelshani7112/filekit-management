 ...
 * Compress Image Page
 * 
 * Purpose: Allow users to compress a single image file with live before/after preview
 * 
 * Features:
 * - Upload single image file (JPG, PNG, WEBP, GIF, BMP, TIFF)
 * - Live before/after slider preview
 * - Compression quality slider (0-100%)
 * - Comprehensive format conversion dropdown
 ...
 * 5. User clicks "Compress Image"
 * 6. File is processed (simulated)
 * 7. User downloads the compressed image
 */
...
const STEPS = [
  ...
  },
  {
    ...
    description: "Download your compressed image instantly. See exactly how much space you saved.",
    icon: Download,
...
const IMAGE_OUTPUT_FORMATS = [
  ...
  { id: "svg", name: "SVG", extension: "svg", category: "Image", description: "Vector" },
  { id: "ico", name: "ICO", extension: "ico", category: "Image", description: "Icon" },
  { id: "heic", name: "HEIC", extension: "heic", category: "Image", description: "iPhone photo" },
...
export default function CompressImagePage({ onWorkStateChange }: CompressImagePageProps = {}) {
  // State management
  ...
  const [outputFormat, setOutputFormat] = useState("original"); // Default: Keep Original
  ...
  const relatedTools = [
    ...
    },
    {
      ...
      icon: FileSignature,
      onClick: () => window.location.href = "/watermark-image",
    },
    {
      name: "Compress PDF",
  ...
  const handleFilesSelected = async (newFiles: File[]) => {
    ...
    if (!['.jpg', '.jpeg', '.png', '.webp', '.gif', '.bmp', '.tiff', '.svg', '.ico', '.heic', '.avif'].includes(ext)) {
  ...
  const handleProcessFiles = async () => {
    ...
    const mockImageContent = `Compressed Image: ${file.name}\nOriginal Size: ${file.size} bytes\nCompressed Size: ${compressedSize} bytes\nCompression Level: ${compressionLevel}%`;
  ...
  return (
    <>
      {/* SEO Meta Tags */}
      <SeoHead path="/compress-image" />
      <ToolJsonLd path="/compress-image" />
      ...
      {currentStep !== "complete" && currentStep !== "edit" && (
        <ToolPageHero 
          ...
          description="Reduce image file size without losing quality. Optimize your images for faster sharing, storage, and web publishing — completely free and secure."
        />
      )}
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <input
                  type="file"
                  id="replaceFiles"
                  accept=".jpg,.jpeg,.png,.webp,.gif,.bmp,.tiff,.svg,.ico,.heic,.avif"
                  className="hidden"
                  onChange={(e) => {
      ...
      <ToolPageLayout>
        ...
        <div className="bg-card border border-border rounded-xl p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
          ...
          {currentStep === "upload" && (
            <>
              <FileUploader
                ...
                acceptedTypes=".jpg,.jpeg,.png,.webp,.gif,.bmp,.tiff,.svg,.ico,.heic,.avif"
                multiple={false}
                maxFiles={1}
                maxFileSize={50}
                fileTypeLabel="Image"
                helperText="JPG, PNG, WEBP, GIF, BMP, TIFF, SVG, ICO, HEIC, AVIF · 1 file · 50MB max"
          ...
          {currentStep === "processing" && (
            <>
              ...
              <RelatedToolsSection 
                tools={relatedTools}
                introText="These tools work well with image compression and help you manage or convert your images."
              />
            </>
          )}
        ...
        {currentStep === "complete" && (
          <RelatedToolsSection 
            tools={relatedTools}
            introText="Continue working with your images or explore other powerful tools."
          />
        )}
        ...
        {currentStep !== "complete" && (
          <>
            ...
            <ToolDefinitionSection
              ...
              content="Image compression reduces the file size of an image by optimizing encoding and removing unnecessary data. It makes images easier to share via email, faster to load on websites, and requires less storage space while maintaining visual quality. WorkflowPro processes your images quickly and keeps everything private."
            />

            {/* How to Use Section - Now inside layout, fits between ads */}
            <HowItWorksSteps 
              title="How It Works"
              subtitle="Compress your image file in four simple steps with our intuitive interface"
              introText="Follow these simple steps to reduce your image file size quickly and securely."
              steps={STEPS} 
            />
            ...
            <UseCasesSection
              title="Popular Uses for Compressing Images"
              useCases={[
                ...
                "Optimize blog post images for SEO",
                "Reduce bandwidth usage for image galleries",
              ...
              faqs={[
                {
                  ...
                  answer: "Compression results vary based on image type and quality setting. At 80% compression, most images reduce by 50-70% while maintaining excellent visual quality. Higher compression levels save more space but may introduce visible artifacts.",
                },
                {
                  question: "What's the difference between compression levels?",
                  answer: "0% = No compression (original quality). 25% = Light compression with minimal quality loss. 50% = Balanced compression. 75% = High compression with good quality. 100% = Maximum compression with acceptable quality for web use.",
                },
                {
                  question: "Should I change the output format?",
                  answer: "By default, we keep your original format. However, WEBP offers the best compression ratios with excellent quality and is ideal for web use. JPG is best for photos, while PNG supports transparency.",
                },
                {
                ...
                },
                {
                  ...
                  answer: "Yes, the maximum file size is 50MB per image. You can compress one image at a time to ensure optimal quality and performance.",
                },
                {
                  question: "Are my images secure during compression?",
                  answer: "Yes, all compression happens locally in your browser. Images are never uploaded to our servers, ensuring complete privacy.",
                },
                {
                  question: "What formats are supported?",
                  answer: "We support JPG, PNG, WEBP, GIF, BMP, TIFF, SVG, ICO, HEIC, and AVIF. You can also convert between these formats during compression.",
                },
                {
            ...
            <ToolSEOFooter
              title="About WorkflowPro's Compress Image Tool"
              content="WorkflowPro's Compress Image tool helps you reduce image file sizes quickly and securely with a live before/after preview. Support for all major image formats with adjustable compression levels (0-100%) and format conversion. Choose your quality settings, see real-time previews, and save storage space — fast, simple, and always free."
            />
          </>
        )}
      </ToolPageLayout>
