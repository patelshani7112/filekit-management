 ...
 * Compress JPG Page
 * 
 * Purpose: Allow users to compress JPG/JPEG image files with live before/after preview
 * 
 * Features:
 * - Upload single JPG/JPEG file only
 ...
 * 
 * How it works:
 * 1. User uploads JPG/JPEG file
 * 2. Live preview shows original vs compressed with draggable slider
 * 3. User adjusts compression level (0-100%)
 * 4. User selects output format (defaults to JPG)
 * 5. User clicks "Compress JPG"
 * 6. File is processed (simulated)
 * 7. User downloads the compressed JPG
 */
...
const STEPS = [
  {
    ...
    description: "Select a JPG/JPEG file from your device or drag and drop it into the upload area.",
    icon: Upload,
  ...
  },
  {
    ...
    description: "Download your compressed JPG instantly. See exactly how much space you saved.",
    icon: Download,
    iconBgColor: "from-green-400 to-green-500",
  },
];

// Available output formats for JPG
const JPG_OUTPUT_FORMATS = [
...
interface CompressJPGPageProps {
  onWorkStateChange?: (hasWork: boolean) => void;
}

export default function CompressJPGPage({ onWorkStateChange }: CompressJPGPageProps = {}) {
  // State management
  ...
  const [outputFormat, setOutputFormat] = useState("jpg"); // Default: JPG
  ...
  const selectedFormat = JPG_OUTPUT_FORMATS.find(f => f.id === outputFormat) || JPG_OUTPUT_FORMATS[0];

  // Filter formats based on search
  const filteredFormats = JPG_OUTPUT_FORMATS.filter(format =>
  ...
  useEffect(() => {
    if (files.length === 1) {
      const baseName = files[0].name.replace(/\.(jpg|jpeg)$/i, '');
  ...
  const relatedTools = [
    ...
    },
    {
      ...
      icon: RefreshCw,
      onClick: () => window.location.href = "/jpg-to-webp",
    },
    {
      name: "Resize Image",
  ...
  const handleFilesSelected = async (newFiles: File[]) => {
    ...
    if (!['.jpg', '.jpeg'].includes(ext)) {
      setValidationMessage(`Only JPG/JPEG files are allowed.`);
  ...
  const handleProcessFiles = async () => {
    ...
    const mockImageContent = `Compressed JPG: ${file.name}\nOriginal Size: ${file.size} bytes\nCompressed Size: ${compressedSize} bytes\nCompression Level: ${compressionLevel}%`;
  ...
  return (
    <>
      {/* SEO Meta Tags */}
      <SeoHead path="/compress-jpg" />
      <ToolJsonLd path="/compress-jpg" />
      
      {/* Navigation Blocker - Warns user before leaving with unsaved work */}
      <NavigationBlocker
        when={hasUnsavedWork}
        message="You have uploaded a JPG file that hasn't been processed yet. If you leave now, all your work will be lost."
      ...
      {currentStep === "complete" && (
        <SuccessHeader
          title="JPG Compressed Successfully!"
          description="Your JPG image has been compressed and is ready to download"
        />
      )}
      ...
      {currentStep !== "complete" && currentStep !== "edit" && (
        <ToolPageHero 
          title="Compress JPG" 
          description="Reduce JPG file size without losing quality. Optimize your JPG photos for faster sharing, storage, and web publishing — completely free and secure."
        />
      )}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-3">
                <p className="text-xs text-blue-700">
                  ...
                  Adjust the compression level above to see real-time changes.
                </p>
              </div>
            </div>
          </div>
      ...
      <ToolPageLayout>
        ...
        <div className="bg-card border border-border rounded-xl p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
          ...
          {currentStep === "upload" && (
            <>
              <FileUploader
                ...
                maxFileSize={50}
                fileTypeLabel="JPG"
                helperText="JPG, JPEG · 1 file · 50MB max"
          ...
          {currentStep === "processing" && (
            <>
              ...
              <ProcessingModal
                ...
                progress={progress}
                title="Compressing JPG..."
                description="Please wait while we compress your JPG image"
                icon={Archive}
              />
              ...
              <RelatedToolsSection 
                tools={relatedTools}
                introText="These tools work well with JPG compression and help you manage or convert your images."
              />
            </>
          )}
          ...
          {currentStep === "complete" && (
            <CompressSuccessSection
              files={downloadUrls}
              onReset={handleReset}
              resetButtonText="Compress Another JPG"
              title="JPG Compressed Successfully!"
              description="Your JPG image has been compressed and is ready to download"
              icon={Archive}
            />
        ...
        {currentStep === "complete" && (
          <RelatedToolsSection 
            tools={relatedTools}
            introText="Continue working with your JPG images or explore other powerful tools."
          />
        )}
        ...
        {currentStep !== "complete" && (
          <>
            ...
            <ToolDefinitionSection
              title="What Is JPG Compression?"
              content="JPG compression reduces the file size of JPG/JPEG images by optimizing encoding and removing unnecessary data. JPG is the most popular photo format, making compression essential for email sharing, website optimization, and storage management. WorkflowPro processes your JPG files quickly while maintaining excellent visual quality and complete privacy."
            />

            {/* How to Use Section - Now inside layout, fits between ads */}
            <HowItWorksSteps 
              title="How It Works"
              subtitle="Compress your JPG file in four simple steps with our intuitive interface"
              introText="Follow these simple steps to reduce your JPG file size quickly and securely."
              steps={STEPS} 
            />
            ...
            <UseCasesSection
              title="Popular Uses for Compressing JPG Images"
              useCases={[
                "Optimize JPG photos for faster website loading",
                "Reduce email attachment sizes for photo sharing",
                "Prepare JPG images for social media posting",
                "Save storage space on your device or cloud",
                "Compress product photos for e-commerce sites",
                "Optimize blog post images for better SEO",
                "Reduce bandwidth usage for photo galleries",
                "Prepare JPG images for mobile apps",
              ]}
            />
            ...
            <ToolFAQSection
              faqs={[
                {
                  question: "How much can I compress JPG images?",
                  answer: "JPG compression is very effective. At 80% compression, most JPG images reduce by 50-70% while maintaining excellent visual quality. Higher compression levels save more space but may introduce visible artifacts. JPG is already a compressed format, so results depend on the original file.",
                },
                {
                  question: "What's the difference between compression levels?",
                  answer: "0% = No compression (original quality). 25% = Light compression with minimal quality loss. 50% = Balanced compression. 75% = High compression with good quality. 100% = Maximum compression with acceptable quality for web use. We recommend 70-85% for most photos.",
                },
                {
                  question: "Can I convert JPG to other formats?",
                  answer: "Yes! You can convert JPG to PNG (for transparency), WEBP (for better compression), GIF, BMP, TIFF, or AVIF during compression. Just select your desired output format from the dropdown.",
                },
                {
                ...
                },
                {
                  ...
                  answer: "Yes, the maximum file size is 50MB per JPG image. You can compress one JPG at a time to ensure optimal quality and performance.",
                },
                {
                  question: "Are my JPG files secure during compression?",
                  answer: "Yes, all compression happens locally in your browser. JPG files are never uploaded to our servers, ensuring complete privacy.",
                },
                {
                  question: "What's the best compression level for JPG?",
                  answer: "For most JPG photos, we recommend 70-85% compression. This provides excellent file size reduction while maintaining high visual quality. For web use, 80-90% works well. For printing, stick to 50-70%.",
                },
                {
                  question: "Is WorkflowPro's JPG compression free?",
            ...
            <ToolSEOFooter
              title="About WorkflowPro's Compress JPG Tool"
              content="WorkflowPro's Compress JPG tool helps you reduce JPG file sizes quickly and securely with a live before/after preview. Perfect for photographers, web designers, and anyone working with JPG photos. Adjustable compression levels (0-100%) with format conversion support. Choose your quality settings, see real-time previews, and save storage space — fast, simple, and always free."
            />
          </>
        )}
      </ToolPageLayout>
