 ...
 * Resize PNG Page
 * 
 * Purpose: Allow users to resize PNG image files with live preview
 * 
 * Features:
 ...
 * 5. User clicks "Resize PNG"
 * 6. File is processed (simulated)
 * 7. User downloads the resized PNG image
 */
...
const STEPS = [
  {
    ...
    description: "Select a PNG image file from your device or drag and drop it into the upload area.",
    icon: Upload,
  ...
  },
  {
    ...
    description: "See a live preview of your resized PNG image with new dimensions displayed.",
    icon: Maximize,
  ...
  },
  {
    ...
    description: "Download your resized PNG image instantly in your preferred format.",
    icon: Download,
...
const IMAGE_OUTPUT_FORMATS = [
  { id: "png", name: "PNG", extension: "png", category: "Default", description: "Keep as PNG" },
  { id: "jpg", name: "JPG / JPEG", extension: "jpg", category: "Image", description: "Convert to JPG" },
  { id: "webp", name: "WEBP", extension: "webp", category: "Image", description: "Modern web format" },
  { id: "gif", name: "GIF", extension: "gif", category: "Image", description: "Animation format" },
...
interface ResizePngPageProps {
  onWorkStateChange?: (hasWork: boolean) => void;
}

export default function ResizePngPage({ onWorkStateChange }: ResizePngPageProps = {}) {
  // State management
  ...
  const relatedTools = [
    ...
    {
      name: "Resize Image",
      description: "Resize any image format",
      icon: Maximize2,
  ...
  return (
    <>
      {/* SEO Meta Tags */}
      <SeoHead path="/resize-png" />
      <ToolJsonLd path="/resize-png" />
      ...
      {currentStep === "complete" && (
        <SuccessHeader
          title="PNG Resized Successfully!"
          description="Your PNG image has been resized and is ready to download"
        />
      )}
      ...
      {currentStep !== "complete" && currentStep !== "edit" && (
        <ToolPageHero 
          title="Resize PNG" 
          description="Resize PNG images with precision while preserving transparency. Change dimensions by pixels or percentage, use social media presets, maintain aspect ratio — completely free and secure."
        />
      )}
        ...
        <ToolPageLayout>
          ...
          <div className="bg-card border border-border rounded-xl p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
            {currentStep === "upload" && (
              <>
                <FileUploader
                  ...
                  helperText="PNG only · 1 file · 50MB max"
            ...
            {currentStep === "processing" && (
              <>
                <ProcessingModal isOpen={true} progress={progress} title="Resizing PNG..." description="Please wait while we resize your PNG image" icon={Maximize2} />
                <RelatedToolsSection tools={relatedTools} introText="These tools work well with PNG resizing." />
              </>
            )}
            ...
            {currentStep === "complete" && (
              <CompressSuccessSection
                files={downloadUrls}
                onReset={handleReset}
                resetButtonText="Resize Another PNG"
                title="PNG Resized Successfully!"
                description="Your PNG image has been resized and is ready to download"
                icon={Maximize2}
              />
          ...
          {currentStep === "complete" && <RelatedToolsSection tools={relatedTools} introText="Continue working with your PNG images." />}

          {currentStep !== "complete" && (
            <>
              <RelatedToolsSection tools={relatedTools} introText="These tools work well with PNG resizing." />
              <ToolDefinitionSection
                title="What Is PNG Resizing?"
                content="PNG resizing allows you to change the dimensions (width and height) of PNG images while preserving transparency and quality. Perfect for web graphics, logos, social media, or reducing file size — completely free, secure, and private."
              />
              <HowItWorksSteps title="How It Works" subtitle="Resize your PNG image in four simple steps" introText="Follow these steps to resize your PNG image quickly and securely." steps={STEPS} />
              ...
              <UseCasesSection
                title="Popular Uses for PNG Resizing"
                useCases={[
                  "Resize PNG images for social media profiles and posts",
                  "Reduce PNG dimensions for faster website loading",
                  "Create thumbnails from large PNG graphics",
                  "Fit PNG logos to specific size requirements",
                  "Scale PNG images for email signatures",
                  "Prepare PNG graphics for printing at specific sizes",
                  "Optimize PNG images for mobile apps and websites",
                  "Batch resize PNG product photos for e-commerce",
                  "Create consistent PNG icon sizes for UI design",
                ]}
              />
              <ToolFAQSection
                faqs={[
                  { question: "Can I resize PNG files with transparency?", answer: "Yes! Our tool fully preserves PNG transparency (alpha channel) during resizing, perfect for logos and graphics." },
                  ...
                  { question: "Does resizing reduce PNG quality?", answer: "Scaling down (making smaller) preserves quality well. Scaling up (making larger) may reduce sharpness but transparency is always preserved." },
                  { question: "Is my PNG file uploaded to a server?", answer: "No — all resizing happens locally in your browser. Your files never leave your device." },
                ]}
              />
              <ToolSEOFooter
                title="About WorkflowPro's Resize PNG Tool"
                content="WorkflowPro's PNG resizer: change dimensions by pixels or percentage, maintain aspect ratio, preserve transparency, use social media presets (Instagram, Facebook, Twitter, YouTube), convert formats, live preview — perfect for web graphics, logos, social media. Fast, simple, always free."
              />
            </>
          )}
        </ToolPageLayout>
