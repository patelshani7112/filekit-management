 ...
 * Word to PDF Page
 * 
 * Purpose: Convert Word documents (.doc, .docx) to PDF format
...
} from "../../../content/tools/pdf-tools/convert-to-pdf/word-to-pdf-content";
...
export default function WordToPdfPage({ onWorkStateChange }: WordToPdfPageProps = {}) {
  // State management
  ...
  // Validate Word file (check for .doc or .docx extension)
  const validateWordFile = async (file: File): Promise<FileValidationInfo> => {
    ...
    if (!validExtensions.includes(ext)) {
      return {
        ...
        isValidating: false,
        isValid: false,
        error: "Invalid file type. Only .doc, .docx, .rtf, and .odt files are supported.",
        uploadProgress: 100,
      };
    }
    
    // Simulate validation - in real app, this would parse the document
    return {
      file,
      isValidating: false,
      isValid: true,
  ...
  // Handle file selection with Word validation
  const handleFilesSelected = async (newFiles: File[]) => {
    ...
    // Validate file types (Word only)
    const invalidFiles = newFiles.filter(file => {
      const ext = '.' + file.name.split('.').pop()?.toLowerCase();
      return ext !== '.doc' && ext !== '.docx' && ext !== '.rtf' && ext !== '.odt';
    });
    ...
    // Validate each Word file with progress animation
    filesToAdd.forEach(async (file, index) => {
      ...
      try {
        // Validate Word file
        const validationResult = await validateWordFile(file);
        ...
        setFileValidationInfo((prev) => {
          ...
          updated[fileIndex] = {
            ...
            isValid: false,
            pageCount: 0,
            error: "Failed to validate Word document",
          };
          return updated;
        });
      }
  ...
  const handleConvert = async () => {
    ...
    const converted = fileValidationInfo
      ...
      .map(info => ({
        fileName: info.file.name.replace(/\.(doc|docx|rtf|odt)$/i, `.${selectedOutputFormat.extension}`),
  ...
  const totalPages = fileValidationInfo.reduce((sum, info) => sum + (info.pageCount || 0), 0);
  ...
  return (
    <>
      <SeoHead
        title="Word to PDF Converter - Convert DOC & DOCX to PDF Free | WorkflowPro"
        description="Convert Word documents to PDF online for free. Convert .doc and .docx files to PDF with perfect formatting in seconds – no signup required."
      ...
      <ProcessingModal
        ...
        description={`Converting your Word documents to ${selectedOutputFormat.name} format`}
        icon={RefreshCw}
      />
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <Button
                  ...
                  onClick={() => document.getElementById('replaceFileWord')?.click()}
              ...
              <p className="text-sm text-gray-600 mb-6">
                Configure how your Word documents will be converted
              </p>
                ...
                <div className="p-4 border-2 border-purple-200 rounded-xl bg-gradient-to-br from-purple-50/50 to-pink-50/50 space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">Keep original document formatting</p>
                    </div>
                    <button
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <ul className="text-xs text-blue-700 leading-relaxed space-y-1">
                        <li> {files.length} Word {files.length === 1 ? 'document' : 'documents'}</li>
              ...
              {fileValidationInfo.map((info, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="aspect-[3/4] bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex flex-col items-center justify-center p-4 relative">
                      {/* File Icon */}
                      <div className="mb-3">
                      ...
                      <div className="text-center w-full px-2">
                        ...
                        <div className="flex items-center justify-center gap-1.5 text-[10px] sm:text-xs text-gray-500">
                          <span>{info.pageCount || 1} {info.pageCount === 1 ? 'page' : 'pages'}</span>
                          <span>•</span>
        ...
        <>
          ...
          {currentStep === "complete" && (
            <SuccessHeader
              ...
              message={files.length === 1 ? `Word document converted to ${selectedOutputFormat.name} successfully!` : `${files.length} Word documents converted to ${selectedOutputFormat.name} successfully!`}
            />
          )}
              ...
              {currentStep === "complete" && (
                <ToolSuccessSection
                  ...
                  fileInfo={{
                    ...
                    'Total Pages': `${convertedFiles.reduce((sum, f) => sum + f.pageCount, 0)} pages`,
            ...
            {currentStep !== "complete" && (
              <>
                ...
                <RelatedToolsSection
                  ...
                  tools={relatedTools}
                  introText="Convert between different document formats quickly and easily."
                />
                ...
                <WhyChooseSection
                  ...
                  subtitle="The most powerful and user-friendly Word to PDF converter available online"
                  introText="WorkflowPro delivers fast, accurate, and secure document conversion trusted by professionals, students, and businesses. No signup required."
