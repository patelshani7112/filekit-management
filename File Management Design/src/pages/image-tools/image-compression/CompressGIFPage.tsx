 ...
 * Compress GIF Page
 * 
 * Purpose: Allow users to compress GIF image files with live before/after preview
 * 
 * Features:
 * - Upload single GIF file only
...
import { 
  ...
  FilePlus, ImageIcon, Maximize2, Crop, ChevronDown, Search, Check, Film
} from "lucide-react";
...
const STEPS = [
  {
    ...
    description: "Select a GIF file from your device or drag and drop it into the upload area.",
    icon: Upload,
  ...
  },
  {
    ...
    description: "Download your compressed GIF instantly. See exactly how much space you saved.",
    icon: Download,
    iconBgColor: "from-green-400 to-green-500",
  },
];

// Available output formats for GIF
const GIF_OUTPUT_FORMATS = [
  ...
  { id: "mp4", name: "MP4", extension: "mp4", category: "Video", description: "Video format" },
];

interface CompressGIFPageProps {
  onWorkStateChange?: (hasWork: boolean) => void;
}

export default function CompressGIFPage({ onWorkStateChange }: CompressGIFPageProps = {}) {
  // State management
  ...
  const [outputFormat, setOutputFormat] = useState("gif"); // Default: GIF
  ...
  const selectedFormat = GIF_OUTPUT_FORMATS.find(f => f.id === outputFormat) || GIF_OUTPUT_FORMATS[0];

  // Filter formats based on search
  const filteredFormats = GIF_OUTPUT_FORMATS.filter(format =>
  ...
  const relatedTools = [
    ...
    {
      name: "Compress PNG",
      description: "Compress PNG images only",
      icon: Archive,
      onClick: () => window.location.href = "/compress-png",
    },
    {
      name: "GIF to MP4",
      description: "Convert GIF animations to MP4 video",
      icon: Film,
      onClick: () => window.location.href = "/gif-to-mp4",
    },
    {
      name: "MP4 to GIF",
      description: "Convert MP4 video to GIF animation",
      icon: Film,
  ...
  // Handle file selection with GIF validation
  const handleFilesSelected = async (newFiles: File[]) => {
    ...
    if (ext !== '.gif') {
      setValidationMessage(`Only GIF files are allowed.`);
  ...
  const handleProcessFiles = async () => {
    ...
    const mockImageContent = `Compressed GIF: ${file.name}\nOriginal Size: ${file.size} bytes\nCompressed Size: ${compressedSize} bytes\nCompression Level: ${compressionLevel}%`;
  ...
  return (
    <>
      {/* SEO Meta Tags */}
      <SeoHead path="/compress-gif" />
      <ToolJsonLd path="/compress-gif" />
      
      {/* Navigation Blocker - Warns user before leaving with unsaved work */}
      <NavigationBlocker
        when={hasUnsavedWork}
        message="You have uploaded a GIF file that hasn't been processed yet. If you leave now, all your work will be lost."
      ...
      {currentStep === "complete" && (
        <SuccessHeader
          title="GIF Compressed Successfully!"
          description="Your GIF animation has been compressed and is ready to download"
        />
      )}
      ...
      {currentStep !== "complete" && currentStep !== "edit" && (
        <ToolPageHero 
          title="Compress GIF" 
          description="Reduce GIF file size without losing animation quality. Optimize your GIF animations for faster sharing, storage, and web publishing — completely free and secure."
        />
      )}
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 border-2 border-blue-200 rounded-lg p-3">
                <p className="text-xs text-blue-700">
                  ...
                  Adjust the compression level above to see real-time changes. GIF animations will still play!
                </p>
              </div>
            </div>
          </div>
      ...
      <ToolPageLayout>
        ...
        <div className="bg-card border border-border rounded-xl p-4 sm:p-6 md:p-8 space-y-4 sm:space-y-6">
          ...
          {currentStep === "upload" && (
            <>
              <FileUploader
                ...
                maxFileSize={50}
                fileTypeLabel="GIF"
                helperText="GIF · 1 file · 50MB max"
          ...
          {currentStep === "processing" && (
            <>
              ...
              <ProcessingModal
                ...
                progress={progress}
                title="Compressing GIF..."
                description="Please wait while we compress your GIF animation"
                icon={Archive}
              />
              ...
              <RelatedToolsSection 
                tools={relatedTools}
                introText="These tools work well with GIF compression and help you manage or convert your animations."
              />
            </>
          )}
          ...
          {currentStep === "complete" && (
            <CompressSuccessSection
              files={downloadUrls}
              onReset={handleReset}
              resetButtonText="Compress Another GIF"
              title="GIF Compressed Successfully!"
              description="Your GIF animation has been compressed and is ready to download"
              icon={Archive}
            />
        ...
        {currentStep === "complete" && (
          <RelatedToolsSection 
            tools={relatedTools}
            introText="Continue working with your GIF animations or explore other powerful tools."
          />
        )}
        ...
        {currentStep !== "complete" && (
          <>
            ...
            <ToolDefinitionSection
              title="What Is GIF Compression?"
              content="GIF compression reduces the file size of GIF animations by optimizing frame data, removing unnecessary metadata, and reducing color palettes while preserving animation. GIF is the most popular format for simple animations, memes, and short clips. WorkflowPro processes your GIF files quickly while maintaining animation smoothness and complete privacy."
            />

            {/* How to Use Section - Now inside layout, fits between ads */}
            <HowItWorksSteps 
              title="How It Works"
              subtitle="Compress your GIF file in four simple steps with our intuitive interface"
              introText="Follow these simple steps to reduce your GIF file size quickly and securely."
              steps={STEPS} 
            />
            ...
            <UseCasesSection
              title="Popular Uses for Compressing GIF Animations"
              useCases={[
                "Optimize GIF animations for faster website loading",
                "Reduce email attachment sizes for animated content",
                "Prepare GIFs for social media sharing (Twitter, Discord, Slack)",
                "Save storage space on your device or cloud storage",
                "Compress reaction GIFs and memes for messaging apps",
                "Optimize blog post animations for better SEO performance",
                "Reduce bandwidth usage for GIF galleries and portfolios",
                "Prepare animated GIFs for mobile apps and presentations",
              ]}
            />
            ...
            <ToolFAQSection
              faqs={[
                {
                  question: "How much can I compress GIF animations?",
                  answer: "GIF compression is very effective. At 80% compression, most GIF animations reduce by 50-70% while maintaining animation smoothness and visual quality. Higher compression levels save more space but may reduce frame quality. Results depend on the number of frames and color complexity.",
                },
                {
                  question: "Will compression affect GIF animation?",
                  answer: "The animation will continue to play smoothly! Compression optimizes frame data and color palettes without removing frames. Higher compression may slightly reduce visual quality per frame, but the animation speed and flow remain the same.",
                },
                {
                  question: "What's the difference between compression levels?",
                  answer: "0% = No compression (original quality). 25% = Light compression with minimal quality loss. 50% = Balanced compression. 75% = High compression with good quality. 100% = Maximum compression with acceptable quality for web use. We recommend 70-85% for most GIF animations.",
                },
                {
                  question: "Can I convert GIF to other formats?",
                  answer: "Yes! You can convert GIF to PNG (first frame only), JPG (first frame), WEBP (animated or static), MP4 (video format), BMP, or TIFF during compression. Just select your desired output format from the dropdown.",
                },
                {
                  question: "Can I see the quality before downloading?",
                  answer: "Yes! Our before/after slider lets you compare the original and compressed versions in real-time. Drag the slider to see the exact quality difference before downloading. The GIF animation will play in both views.",
                },
                {
                  question: "Is there a file size limit?",
                  answer: "Yes, the maximum file size is 50MB per GIF animation. You can compress one GIF at a time to ensure optimal quality and performance.",
                },
                {
                  question: "Are my GIF files secure during compression?",
                  answer: "Yes, all compression happens locally in your browser. GIF files are never uploaded to our servers, ensuring complete privacy.",
                },
                {
                  question: "What's the best compression level for GIF?",
                  answer: "For most GIF animations, we recommend 70-85% compression. This provides excellent file size reduction while maintaining animation quality. For social media, 80-90% works well. For high-quality portfolios, stick to 50-70%.",
                },
                {
                  question: "Is WorkflowPro's GIF compression free?",
            ...
            <ToolSEOFooter
              title="About WorkflowPro's Compress GIF Tool"
              content="WorkflowPro's Compress GIF tool helps you reduce GIF animation file sizes quickly and securely with a live before/after preview while preserving animation smoothness. Perfect for social media managers, meme creators, and anyone working with GIF animations. Adjustable compression levels (0-100%) with format conversion support including MP4. Choose your quality settings, see real-time previews, maintain animation flow, and save storage space — fast, simple, and always free."
            />
          </>
        )}
      </ToolPageLayout>
